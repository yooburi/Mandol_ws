...
#define ACCEL_DB_US             25     // ±25us (수신 환경에 맞게 20~50us 조정)
#define STEER_DB_US             25

// 2) 정규화(-1..+1) 데드밴드
#define RC_THROTTLE_DB_NORM     0.08   // 스로틀 소신호 무시
#define RC_STEER_DB_NORM        0.1   // 스티어 소신호 무시
...
float  throttle_cmd = 0.0f;      // -1..+1
float  steer_auto_deg = 0.0f;    // -MAX..+MAX [deg]
...
void loop() {
  ...
  float Steer_input    = Mapping(Steering_val, 1280.0f, 1792.0f, -1.0f, 1.0f);
  ...
  // ★ 센서 dead band 적용(중립 부근 미세 잡음 억제)
  ...
  if (Mode_val == BREAK_MODE) {
    ...
    // 리모컨 스로틀 데드밴드가 적용된 Throttle_input 사용
    if (Throttle_input > 0.0)      MoveForward(Throttle_input);
    else if (Throttle_input < 0.0) MoveBackward(-Throttle_input);
    else { digitalWrite(DIR1, HIGH); analogWrite(PWM1, 0); digitalWrite(DIR2, HIGH); analogWrite(PWM2, 0); }

    // 스티어: 리모컨 스티어 데드밴드 → ref_steer_deg_rc → PID(u) → Steer(-u)
    Steer(-u);
  }
  else if (Mode_val == AUTO_MODE) {
    // === [삭제됨] SA 신선도 체크 ===
    // bool sa_ok_now = steerFresh && (millis() - lastSteerMs <= STEER_TIMEOUT_MS);
    // if (!sa_ok_now) {
    //   StopMotor();
    //   return;
    // }

    // 1) 고정 속도(비율 0.0~1.0)로 전진
    const float AUTO_FIXED_THR = 0.4f;  // ★ 원하는 속도로 조정
    MoveForward(AUTO_FIXED_THR);

    // 2) 조향: SA 각도를 목표로 PID → Steer
    double ref_auto = (double)steer_auto_deg;
    double u_auto = PID(ref_auto, deg, dt_us);

    // 출력 dead band
    u_auto = applyDeadband(u_auto, PID_DEADBAND);
    if (u_auto > 1.0) u_auto = 1.0; 
    else if (u_auto < -1.0) u_auto = -1.0;
