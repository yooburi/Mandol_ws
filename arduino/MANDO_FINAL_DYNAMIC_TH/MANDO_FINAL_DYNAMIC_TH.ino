...
void loop() {
  ...
  // === 8) 목표각 선택 ===
  double ref_steer_deg = (Mode_val == AUTO_MODE) ? (double)steer_auto_deg : ref_steer_deg_rc;
  ...
  if (Mode_val == BREAK_MODE) {
    ...
    // 스로틀: 시리얼 TH 값 사용 (신선도 체크 없음)
    float th = throttle_cmd;
    if (th > 0.0f)        MoveForward(th);
    else if (th < 0.0f)   MoveBackward(-th);
    else                  StopDriveOnly();

    // 조향: 시리얼 SA 값 사용 (신선도 체크 없음)
